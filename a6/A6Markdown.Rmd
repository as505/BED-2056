...
To see the effect of Covid-19 on bankruptcies in Norway, we looked at data from Brønnøysund to compare
bankruptcies this year and 2019. Due to Brønnøysunds 5000-hit search limit, the data was limited to between April and late August.
April was chosen to give Covid some time to affect companies economy, since most businesses would likely not go bankrupt immediately. 
April-August gave a good time frame for when companies would struggle economically while lockdown was in effect
```{r main_code, echo = FALSE}
...
fin_df <- bind_rows(Oslo, Viken, Trøndelag, Rogaland, Innlandet, Troms_og_Finnmark, Møre_og_Romsdal, Vestfold_og_telemark, Nordland, Agder, Svalbard, Vestland)
...
fin_df2 <- bind_rows(Oslo, Viken, Trøndelag, Rogaland, Innlandet, Troms_og_Finnmark, Møre_og_Romsdal, Vestfold_og_telemark, Nordland, Agder, Svalbard, Vestland)
# Combine cumulative for all regions from 2020 data
```

## Data Plot
Regions without data not shown:
...
ggplot() + 
  ...
  ggtitle("“Commencement of liquidation proceedings” 2019 (Green) VS 2020 (Red)") +
  ylab("Cumulative Amount") + xlab("Time period: 01.April - 30.August") + facet_wrap(~Fylke)
```

For the time being, it seems Covid has yet to significantly affect the number of bankruptcies
